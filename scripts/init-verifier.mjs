 ...
 *
 * The stellar CLI cannot pass complex custom struct types (like VerifyingKey)
 * via JSON, so this script uses the JS SDK to directly construct and send
 * the init transaction.
 *
 * Usage: node scripts/init-verifier.mjs [--dry-run]
 */

import { rpc, TransactionBuilder, BASE_FEE, Contract, Address, xdr, nativeToScVal, Keypair } from '@stellar/stellar-sdk'
...
// ─── Config (mirrors frontend/src/config) ─────────────────────────────────────
...
function bytesVal(hex, byteLen = 32) {
  return xdr.ScVal.scvBytes(hexToU8(hex, byteLen))
}
...
function g2Point(point) {
  ...
  // Soroban: x_im || x_re || y_im || y_re (each 32 bytes)
...
async function main() {
  ...
  if (dryRun) {
    ...
    console.log('VKey ScVal type:', vkScVal.switch().name)
    return
  }
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (status.status === rpc.Api.GetTransactionStatus.FAILED) {
      ...
      console.error('Result:', JSON.stringify(status.resultXdr))
      process.exit(1)
    }
